...
fn main() {
	...
	// let arc_xs : Arc<Mutex<&Vec<usize>>> = Arc::new()

	let xs_clone = xs.clone();
	// let xs_ptr1 = Arc::clone(&xs[0]);
	let t1 = thread::spawn(move || map_data(&xs[0]));

	// let xs_ptr2 = Arc::clone(&xs[1]);
	let t2 = thread::spawn(move || map_data(&xs_clone[1]));
	
	
	let join_t1 = t1.join().unwrap();
	intermediate_sums.push(join_t1);

	let join_t2 = t2.join().unwrap();
	intermediate_sums.push(join_t2);
	...
	//partition data into n=num_parition parts
	...
	for i in 0..num_partitions {
		...
		let curr_thread = thread::spawn(move || map_data(&xs2_clone[i]));
		handles.push(curr_thread);
	}
	for curr_thread in handles {
		let curr_threadjoin = curr_thread.join().unwrap();

        // Pushes the map_data result to the intermediate_sums_2 vector
        intermediate_sums2.push(curr_threadjoin);
	}
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
	...
	let partition_sizex1 : usize = v.len() / num_partitions;
	let mut partition_sizex2 = v.len() % num_partitions;
	// println!("partition_sizex1 {:?}", partition_sizex1);
	// println!("partition_sizex2 {:?}", partition_sizex2);
	
    // Create a vector that will contain vectors of integers
    let mut xs: Vec<Vec<usize>> = Vec::new();
	let mut curr_index = 0;
	let mut curr_size = partition_sizex1;
	for i in 0..num_partitions {
		if partition_sizex2 > 0 {
			curr_size = curr_size+1;
			partition_sizex2 = partition_sizex2-1;
		}
		// println!("curr_size {:?}", curr_size);

		let mut new_vec : Vec<usize> = Vec::new();
		// println!("vec created");
		for k in curr_index..curr_size {
			new_vec.push(v[k]);
			// println!("partition {:?} inserted {:?}",i, v[k]);
			curr_index = k;
		}
		xs.push(new_vec);
		curr_size = curr_size + partition_sizex1;
